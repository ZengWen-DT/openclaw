...
describe("repairMissingConfiguredPluginInstalls", () => {
  ...
  it("does not flag a stale path-install record shadowed by a configured load-path plugin", async () => {
    const stalePath = path.resolve("/missing/fs-utils");
    const loadPathRoot = path.resolve("/srv/plugins/fs-utils");
    const records = {
      "fs-utils": {
        source: "path",
    ...
    const issues = await detectConfiguredPluginInstallHealthIssues({
      cfg: {
        plugins: {
          ...
          entries: { "fs-utils": { enabled: true } },
        },
      },
      env: {},
    });

    expect(issues.filter((issue) => issue.pluginId === "fs-utils")).toEqual([]);
  });

  it("prunes a stale path-install record shadowed by a configured load-path plugin", async () => {
    const stalePath = path.resolve("/missing/stale-utils");
    const loadPathRoot = path.resolve("/srv/plugins/stale-utils");
    const records = {
      "stale-utils": {
        source: "path",
        sourcePath: stalePath,
    ...
    const result = await repairConfiguredPlugins(
      {
        plugins: {
          load: { paths: [loadPathRoot] },
          entries: { "stale-utils": { enabled: true } },
        },
      },
      { OPENCLAW_STATE_DIR: tempDirs.make("openclaw-lease-isolated-") },
    );

    expect(mocks.installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(mocks.updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(result.records["stale-utils"]).toBeUndefined();
    expect(result.changes).toContain(
      'Removed stale path-install record for plugin "stale-utils" (loaded from a configured load path).',
    );
    expect(result.warnings).toEqual([]);
    expect(result.failedPluginIds).toBeUndefined();
  });

  it("does not re-report a stale path-install record even when a download candidate exists", async () => {
    const stalePath = path.resolve("/missing/candidate-utils");
    const loadPathRoot = path.resolve("/srv/plugins/candidate-utils");
    const records = {
      "candidate-utils": {
        source: "path",
        sourcePath: stalePath,
    ...
    // A downloadable official replacement exists for the same id, but the
    // plugin is already healthy from a configured load path, so Doctor must
    // not propose a reinstall for the stale path record.
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      officialPluginEntry({
        id: "candidate-utils",
        npmSpec: "@openclaw/candidate-utils-plugin",
      }),
    ]);
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "candidate-utils",
        pluginId: "candidate-utils",
        meta: { label: "Candidate Utils" },
        install: {
          npmSpec: "@openclaw/candidate-utils-plugin",
        },
      },
    ...
    const issues = await detectConfiguredPluginInstallHealthIssues({
      cfg: {
        plugins: {
          load: { paths: [loadPathRoot] },
          entries: { "candidate-utils": { enabled: true } },
        },
      },
      env: {},
    });

    expect(issues.filter((issue) => issue.pluginId === "candidate-utils")).toEqual([]);
  });
  it("keeps a missing path record when no configured load-path plugin shadows it", async () => {
    const missingDir = path.resolve("/missing/orphan");
    const records = {
      orphan: {
        source: "path",
        sourcePath: missingDir,
        installPath: missingDir,
      },
    };
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
    // No origin "config" plugin discovered: the record stays a reinstallable
    // missing payload just like before.
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      plugins: [{ id: "orphan", origin: "global", rootDir: "/tmp/global/orphan" }],
      diagnostics: [],
    });
